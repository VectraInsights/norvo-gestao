...
export function gerarDactePdf(data: DacteData): Blob {
  ...
  const party = (x: number, title: string, p: { nome?: string; lgr?: string; cid?: string; cep?: string; bai?: string; doc?: string; ie?: string; uf?: string; fone?: string }) => {
    ...
    const labX2 = x + 68, colonX2 = x + 86, valX2 = x + 88;
    ...
    let yy = y + 7;
    lab("Endereço", x + 1.5, yy); col(colonX1, yy); v2(cut(D(p.lgr), 70), valX1, yy); yy += 2.9;
  ...
  ["Nome", "Valor", "Nome", "Valor", "Nome", "Valor"].forEach((h, i) => {
    const cc = Math.floor(i / 2);
    if (i % 2 === 0) doc.text(h, gridX + 1 + cc * colW, y + 3);
    else doc.text(h, gridX + (cc + 1) * colW - 1 - doc.getTextWidth(h), y + 3);
  });
  compRows.forEach((row, r) => {
    row.forEach(([nm, vv], c) => {
      ...
      const vvS = fmtNum(vv);
      doc.text(vvS, gridX + (c + 1) * colW - 1 - doc.getTextWidth(vvS), y + chH + 3.2 + r * crH);
    });
  });
  ...
  { const sV = fmtNum(vBruto); setFont("bold", 7); black(); doc.text(sV, M + CW - 2 - doc.getTextWidth(sV), y + 7.2); }
  setFont("bold", 5.5); black();
  doc.text("Valor à Receber", totX + 2, y + 12.5);
  { const sV = fmtNum(vReceber); setFont("bold", 7); black(); doc.text(sV, M + CW - 2 - doc.getTextWidth(sV), y + 16.2); }
  y += compH + 1;
  ...
  doc.text("RG", M + 2, y + 11.5);
  ctr("Assinatura ou Carimbo", M + 83, y + 12, 5);
  setFont("normal", 4.5); black();
  doc.text("Início de Prestação - Data/Hora", M + 110, y + 6);
  border(); doc.line(M + 109, y + 9, M + 151, y + 9);
  doc.text("Término da Prestação - Data/Hora", M + 110, y + 12);
  setFont("bold", 6); black();
