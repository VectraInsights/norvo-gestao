...
export function calcIRRF(baseCalculo: number, salarioBruto: number): number {
  let imposto = 0;
...
export function calcSalarioLiquido(bruto: number, proventos = 0, outrosDescontos = 0): ResultadoSalario {
  const inss = calcINSS(bruto);
  const baseIrrf = Math.max(0, bruto - inss);
  const irrf = baseIrrf > 0 ? calcIRRF(baseIrrf, bruto) : 0;
...
export function calcFerias(salario: number, diasGozo: number, abonoDias = 0, adiantarDecimo = false): ResultadoFerias {
  ...
  const irrfFerias = calcIRRF(Math.max(0, brutoFerias - inssFerias), brutoFerias);
...
export function calcDecimoTerceiro(salario: number, meses: number): ResultadoDecimo {
  const m = Math.min(12, Math.max(1, Math.round(meses) || 0));
  const bruto = (salario * m) / 12;
  const inss = calcINSS(bruto);
  const irrf = calcIRRF(Math.max(0, bruto - inss), bruto);
