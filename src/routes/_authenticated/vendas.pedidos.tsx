...
// Transições válidas server-side (defesa em profundidade além da UI).
const TRANSICOES: Record<VendaStatus, VendaStatus[]> = {
  rascunho: ["proposta", "pedido", "cancelado"],
  proposta: ["pedido", "cancelado"],
...
function VendasPage() {
  ...
  // Guard optimista + verificação real do status anterior via WHERE.
  // Se o estado mudou entre o clique e o UPDATE (outro usuário/tab), a linha
...
function NovaVendaSheet({ onClose }: { onClose: () => void }) {
  ...
  const totals = useMemo(() => {
    ...
    return { linhas, subtotal, desconto, total: subtotal - desconto };
  }, [itens]);

  // Duas escritas sequenciais (venda + itens) sem transação SQL-level.
  // Se a segunda falhar, deletamos a venda para não deixar registro órfão.
  // Trade-off: se a DELETE também falhar (rede caiu), sobra órfão — aceitável
  // porque RLS impede vazamento e o usuário reprocessa. Alternativa
  // verdadeiramente atômica exigiria RPC PL/pgSQL (próxima onda).
  const salvar = useMutation({
    mutationFn: async (status: "rascunho" | "proposta" | "pedido") => {
      ...
      const payload = itens.map((it, i) => ({
        ...
        desconto: totals.linhas[i]?.desc ?? 0,
        total: totals.linhas[i]?.total ?? 0,
      }));
      const { error: e2 } = await supabase.from("venda_itens").insert(payload);
      if (e2) {
        await supabase.from("vendas").delete().eq("id", venda.id); // compensating rollback
                ...
                <TableBody>
                  {itens.map((it, i) => (
                    <TableRow key={i}>
                      ...
                      <TableCell><Input type="number" step="0.01" min="0" value={it.preco_unitario} onChange={(e) => updateItem(i, { preco_unitario: Number(e.target.value) })} className="h-8" /></TableCell>
