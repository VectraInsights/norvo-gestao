...
ALTER TABLE public.categorias_financeiras
  ADD COLUMN IF NOT EXISTS parent_id UUID REFERENCES public.categorias_financeiras(id) ON DELETE CASCADE;

CREATE INDEX IF NOT EXISTS idx_categorias_financeiras_parent ON public.categorias_financeiras(parent_id);

-- Rewrite defaults trigger with rich categories + subcategories
