-- Autoria dos lançamentos gerados pela recorrência de adiantamentos:
-- o lançamento nasce com created_by = criador do adiantamento recorrente.
-- 1) created_by em adiantamentos passa a ser preenchido automaticamente (auth.uid())
--    nos inserts da UI que não enviam a coluna.
ALTER TABLE public.adiantamentos ALTER COLUMN created_by SET DEFAULT auth.uid();

-- 2) A função de geração replica o criador da recorrência no lançamento.
      ...
      IF r.ultimo_mes_gerado IS NULL OR r.ultimo_mes_gerado < chave THEN
        ...
        INSERT INTO public.lancamentos_financeiros
          (empresa_id, tipo, status, descricao, valor, data_emissao, data_vencimento,
           categoria_id, observacoes, created_by)
        VALUES (
          r.empresa_id, 'pagar', 'aberto',
          COALESCE(r.colab_nome, 'Adiantamento') || ' (recorrência)',
          r.valor, hoje, venc,
          v_cat,
